  ...
  startGrowthAnimation(opts: GrowthAnimationOpts) {
    ...
    setTimeout(() => {
      ...
      const frameListener = (clock) => {
        ...
        // 要在起点和终点之间动态添加点，与初始绘图逻辑一致，使用 newPosition 更新 points 数组中索引处的 movingPointIndex 处的点，
        // 生成箭头，然后执行动画。
        
        // ！通过时间差控制比例，elapsedTime 会不断增大
        const t = (elapsedTime - currentSegment * segmentDuration) / segmentDuration;

        // 计算起点到终点之间，一定比例的线性插值坐标
